...
abstract class CompareAllocations {
	...
	public void runComparison() {
		...
		try {
			...
			System.out.println("Execution cost using direct buffer: <" + (endTime - startTime) + ">");
			startTime = System.nanoTime();
			indirectAllocate();
			endTime = System.nanoTime();
			System.out.println(
					"Indirect Allocation Cost for buffer of size: <" + size + "> is <" + (endTime - startTime) + ">");
			startTime = System.nanoTime();
			execute();
			endTime = System.nanoTime();
			System.out.println("Execution cost using indirect buffer: <" + (endTime - startTime) + ">");
		} catch (IOException e) {
	...
	public static void main(String[] args) {
		CompareAllocations[] comparisons = { new CompareAllocations("GetChannel", 8192) {
			public void execute() throws IOException {
				FileChannel fc = new FileInputStream("./src/io/Ex25_AllocateDirect.java").getChannel();
				fc.read(buffer);
				buffer.flip();
			...
			public void execute() throws IOException {
				FileChannel in = new FileInputStream("./src/io/Ex25_AllocateDirect.java").getChannel(),
						out = new FileOutputStream("./src/io/temp.txt").getChannel();
			...
			public void execute() throws IOException {
				...
				fc.write(ByteBuffer.wrap("./src/io/Some text".getBytes()));
				fc.close();
