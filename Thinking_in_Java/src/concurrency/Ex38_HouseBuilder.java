...
class House {
	...
	private boolean steel = false, concreteForms = false, concreteFoundation = false, plumbing = false,
...
class HouseBuilder implements Runnable {
	...
	public HouseBuilder(HouseQueue hq, HouseQueue fq, TeamPool tp) {
		footingsQueue = hq;
		finishingQueue = fq;
		teamPool = tp;
	}
...
class TeamPool {
	...
	public void release(Team t) {
		add(t);
	}
}

public class Ex38_HouseBuilder {
	public static void main(String[] args) throws Exception {
		...
		exec.execute(new HouseBuilder(footingsQueue, finishingQueue, teamPool));
